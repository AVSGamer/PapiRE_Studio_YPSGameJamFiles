...
const skip_regex_1 = __importDefault(require("skip-regex"));
 ...
 */
function handleSlashExtraction(expr, index) {
    if (expr[index + 1] === '/') {
 ...
 */
const RE_EXPR = RegExp(regexes_1.default.S_STRINGS + '|[`/{}]', 'g');
/**
    ...
    while (mm = re.exec(expr)) {
        switch (mm[0]) {
            ...
            case '`':
                ...
                break;
            case '/':
                result = handleSlashExtraction(expr, mm.index);
                if (result) {
                    return result;
                }
                re.lastIndex = skip_regex_1.default(expr, mm.index);
        }
    }
    return expr;
};
...
const getExpr = function (key, expr) {
    ...
    */
    if (key === 'set') {
        const mm = regexes_1.default.ASSIGNMENT.exec(expr);
        const ss = mm && mm[2];
