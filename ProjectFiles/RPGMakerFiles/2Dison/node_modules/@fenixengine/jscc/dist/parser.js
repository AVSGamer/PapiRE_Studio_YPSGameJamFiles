...
*/
const eval_expr_1 = __importDefault(require("./eval-expr"));
const get_expr_1 = __importDefault(require("./get-expr"));
...
class Parser {
    ...
    _normalize(key, expr) {
        ...
        return get_expr_1.default(key, expr);
    }
    /**
        ...
        if (state === 0 /* WORKING */) {
            switch (key) {
                ...
                case 'unset':
                    ...
                    break;
                case 'error':
                    expr = String(eval_expr_1.default(this.options, expr));
     ...
     */
    _set(expr) {
        const match = expr.match(regexes_1.default.ASSIGNMENT);
        if (match) {
            const varname = match[1];
            const exprStr = match[2] || '';
            this.options.values[varname] = exprStr
                ? eval_expr_1.default(this.options, exprStr.trim()) : undefined;
        }
        else {
     ...
     */
    _unset(varname) {
        if (varname.match(regexes_1.default.VARNAME)) {
    ...
    _getIfValue(key, expr) {
        ...
        if (key === 'if') {
            return eval_expr_1.default(this.options, expr) ? 1 : 0;
        }
