...
var jscc = require('@fenixengine/rollup-plugin-jscc');
...
const __filename$2 = url.fileURLToPath((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.src || new URL('fenix-wizard.cjs', document.baseURI).href)));
const __dirname$1 = path.dirname(__filename$2);

const rootDir = path.dirname(__dirname$1);
...
async function pipeSpawn (command, params, options = {}) {
  return new Promise((resolve, reject) => {
    const cp = crossSpawn.spawn(command, params, {
      env: process.env,
      shell: true,
      ...options
    });
...
async function getConfig (options = {}) {
  ...
  if (typeof options === 'string') {
    configPath = path.resolve(options);
  ...
  configData.target = path.resolve(configData.target);
  configData.output = resolvedOutputs;
  configData.configFilepath = path.resolve(`${configPath}/wizard.config.js`);
...
function createFileWatcher (pathOrPaths, listener) {
  ...
  watcher$1 = chokidar__namespace.watch(paths, {
    persistent: true,
    ignoreInitial: true,
...
function startServer (source = './src', options) {
  ...
  // Setup socket server
  const socketServer = new ws.WebSocketServer({ port: socketPort });
...
_loadCommands(commander.program);

const entryFile = process.argv?.[1];
const __filename$1 = url.fileURLToPath((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.src || new URL('fenix-wizard.cjs', document.baseURI).href)));

if (entryFile === __filename$1 || process.argv.length > 1) {
  commander.program
