...
function subdigestOf(payload) {
  return ethers.solidityPackedKeccak256(['bytes', 'uint256', 'address', 'bytes32'], ['0x1901', payload.chainId, payload.address, payload.digest]);
}
...
function encodeSignature$1(signature) {
  ...
  const encodedSigners = signers.map(s => {
    if (isAddressMember(s)) {
      return ethers.solidityPacked(['uint8', 'uint8', 'address'], [SignaturePartType$1.Address, s.weight, s.address]);
    }
    if (s.isDynamic) {
      const bytes = ethers.getBytes(s.signature);
      return ethers.solidityPacked(['uint8', 'uint8', 'address', 'uint16', 'bytes'], [SignaturePartType$1.DynamicSignature, s.weight, s.address, bytes.length, bytes]);
    }
    return ethers.solidityPacked(['uint8', 'uint8', 'bytes'], [SignaturePartType$1.EOASignature, s.weight, s.signature]);
  });
...
const ConfigCoder$1 = {
  ...
  imageHashOf: config => {
    return config.signers.reduce((imageHash, signer) => ethers.keccak256(ethers.AbiCoder.defaultAbiCoder().encode(['bytes32', 'uint8', 'address'], [imageHash, signer.weight, signer.address])), ethers.solidityPacked(['uint256'], [config.threshold]));
  },
...
const partEncoder = {
  ...
  nested: (weight, threshold, tree) => {
    const arr = ethers.getBytes(tree);
    return ethers.solidityPacked(['uint8', 'uint8', 'uint16', 'uint24', 'bytes'], [SignaturePartType.Nested, weight, threshold, arr.length, arr]);
  },
  subdigest: subdigest => {
  ...
  dynamicSignature: (weight, address, signature) => {
    ...
    return ethers.solidityPacked(['uint8', 'uint8', 'address', 'uint24', 'bytes'], [SignaturePartType.DynamicSignature, weight, address, arrSignature.length, arrSignature]);
  },
...
function encodeSigners(config, parts, subdigests, chainId, options = {}) {
  ...
  if (BigInt(chainId) === 0n) {
    return {
      encoded: ethers.solidityPacked(['uint8', 'uint16', 'uint32', 'bytes'], [SignatureType.NoChainIdDynamic, config.threshold, config.checkpoint, tree.encoded]),
      weight: tree.weight
    };
  }
  if (BigInt(config.threshold) > 255n) {
    return {
      encoded: ethers.solidityPacked(['uint8', 'uint16', 'uint32', 'bytes'], [SignatureType.Dynamic, config.threshold, config.checkpoint, tree.encoded]),
      weight: tree.weight
    };
  }
  return {
    encoded: ethers.solidityPacked(['uint8', 'uint8', 'uint32', 'bytes'], [SignatureType.Legacy, config.threshold, config.checkpoint, tree.encoded]),
    weight: tree.weight
  };
}
...
function setImageHashStruct(imageHash) {
  return ethers.solidityPacked(['bytes32', 'bytes32'], [ethers.solidityPackedKeccak256(['string'], ['SetImageHash(bytes32 imageHash)']), imageHash]);
}
...
function encodeSignatureBody(decoded) {
  return ethers.solidityPacked(['uint16', 'uint32', 'bytes'], [decoded.threshold, decoded.checkpoint, encodeSignatureTree(decoded.tree)]);
}
function encodeSignatureTree(tree) {
  if (isNode(tree) || isUnrecoveredNode(tree)) {
    ...
    if (isBranching) {
      return ethers.solidityPacked(['bytes', 'uint8', 'uint24', 'bytes'], [encodedLeft, SignaturePartType.Branch, encodedRight.length, encodedRight]);
    } else {
  ...
  if (isNestedLeaf(tree) || isUnrecoveredNestedLeaf(tree)) {
    ...
    return ethers.solidityPacked(['uint8', 'uint8', 'uint16', 'uint24', 'bytes'], [SignaturePartType.Nested, tree.weight, tree.threshold, nested.length, nested]);
  }
  if (isUnrecoveredSignatureLeaf(tree) || isSignerLeaf(tree) && tree.signature !== undefined) {
    ...
    if (tree.isDynamic || signature.length !== SignaturePartTypeLength) {
      ...
      return ethers.solidityPacked(['uint8', 'uint8', 'address', 'uint24', 'bytes'], [SignaturePartType.DynamicSignature, tree.weight, tree.address, signature.length, signature]);
    } else {
  ...
  if (isUnrecoveredNode(tree)) {
    ...
    if (left.weight === 0 && right.weight === 0) {
      try {
        ...
        const recovered = await recoverTopology(tree, ethers.ZeroHash, undefined);
        return {
          weight: 0,
          trimmed: {
...
function hashNode(node) {
  ...
  if (isNestedLeaf(node)) {
    ...
    return ethers.solidityPackedKeccak256(['string', 'bytes32', 'uint256', 'uint256'], ['Sequence nested config:\n', nested, node.threshold, node.weight]);
  }
  if (isNodeLeaf(node)) {
...
function imageHash(config) {
  return ethers.solidityPackedKeccak256(['bytes32', 'uint256'], [ethers.solidityPackedKeccak256(['bytes32', 'uint256'], [hashNode(config.tree), config.threshold]), config.checkpoint]);
}
...
function addressOf(context, imageHash) {
  const codeHash = ethers.keccak256(ethers.solidityPacked(['bytes', 'bytes32'], [context.walletCreationCode, ethers.zeroPadValue(context.mainModule, 32)]));
  const hash = ethers.keccak256(ethers.solidityPacked(['bytes1', 'address', 'bytes32', 'bytes32'], ['0xff', context.factory, imageHash, codeHash]));
...
function intendedTransactionID(bundle) {
  return ethers.keccak256(ethers.AbiCoder.defaultAbiCoder().encode(['address', 'uint256', 'bytes32'], [bundle.intent.wallet, bundle.chainId, bundle.intent.id]));
}
...
function subdigestOfGuestModuleTransactions(guestModule, chainId, txs) {
  return subdigestOf({
    address: guestModule,
    chainId,
    digest: ethers.keccak256(ethers.AbiCoder.defaultAbiCoder().encode(['string', MetaTransactionsType], ['guest:', sequenceTxAbiEncode(txs)]))
  });
}
...
function sequenceTxAbiEncode(txs) {
  return txs.map(tx => {
    ...
    return {
      ...
      target: (_tx$to = tx.to) != null ? _tx$to : ethers.ZeroAddress,
...
async function validateEIP6492Offchain(provider, signer, hash, signature) {
  try {
    const result = await provider.call({
      data: ethers.concat([EIP_6492_OFFCHAIN_DEPLOY_CODE, ethers.AbiCoder.defaultAbiCoder().encode(['address', 'bytes32', 'bytes'], [signer, hash, signature])])
    });
    return result === '0x01';
  } catch (err) {
    return false;
