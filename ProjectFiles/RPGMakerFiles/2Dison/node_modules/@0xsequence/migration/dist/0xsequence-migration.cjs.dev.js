...
function counterfactualVersion(address, firstImageHash, versions) {
  for (let i = 0; i < versions.length; i++) {
    if (core.commons.context.addressOf(versions[i], firstImageHash) === address) {
...
class Migration_v1v2 {
  constructor() {
    this.version = 2;
    this.configCoder = core.v2.config.ConfigCoder;
    this.signatureCoder = core.v2.signature.SignatureCoder;
  }
  buildTransaction(address, contexts, newConfig) {
    // If new config is not v2, then we need to convert it to v2
    if (!core.v2.config.ConfigCoder.isWalletConfig(newConfig)) {
      const v2Config = core.v2.config.toWalletConfig({
    ...
    const updateBundle = core.v2.config.ConfigCoder.update.buildTransaction(address, newConfig, context, 'first');
    const tx = {
      entrypoint: address,
      nonce: core.commons.transaction.encodeNonce(MIGRATION_NONCE_SPACE, 0),
      transactions: [{
        to: address,
        value: 0,
        gasLimit: 0,
  ...
  decodeTransaction(tx, contexts) {
    ...
    const decoded2 = core.v2.config.ConfigCoder.update.decodeTransaction({
      entrypoint: address,
