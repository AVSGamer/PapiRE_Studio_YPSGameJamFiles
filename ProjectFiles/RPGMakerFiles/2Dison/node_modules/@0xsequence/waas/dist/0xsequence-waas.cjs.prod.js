...
const VERSION = `${INTENTS_VERSION} (Web ${core.VERSION})`;
let timeDrift;
...
function hashIntent(intent) {
  ...
  const encoded = ethers.ethers.toUtf8Bytes(jsonCanonicalize.canonicalize(hashableIntent));
  return ethers.ethers.getBytes(ethers.ethers.keccak256(encoded));
}
...
function signMessage({
  ...
  return makeIntent(IntentName.signMessage, lifespan, {
    ...
    message: message.startsWith('0x') ? message : ethers.ethers.hexlify(ethers.ethers.toUtf8Bytes(message))
  });
}
  ...
  return makeIntent(IntentName.feeOptions, lifespan, {
    ...
    transactions: transactions.map(tx => {
      ...
      return {
        type: 'transaction',
        to: tx.to,
        value: utils.toHexString(BigInt(tx.value || 0)),
        data: ethers.ethers.hexlify(tx.data || '0x')
      };
    })
  });
}
...
function contractCall(data) {
  ...
  if (sendContractCallArgs.abi !== undefined) {
    return {
      ...
      value: utils.toHexString(BigInt(sendContractCallArgs.value || 0)),
      data: {
...
function delayedEncode(data) {
  ...
  if (sendDelayedEncodeArgs.abi !== undefined) {
    return {
      ...
      value: utils.toHexString(BigInt(sendDelayedEncodeArgs.value)),
      data: {
...
async function newSECP256R1SessionFromSessionId(sessionId, cryptoBackend, secureStoreBackend) {
  ...
  return {
    sessionId: async () => {
      ...
      return ethers.ethers.hexlify(pubKeyTypedRaw);
    },
    sign: async message => {
      if (typeof message === 'string') {
        if (message.startsWith('0x')) {
          message = message.slice(2);
          message = ethers.ethers.getBytes(message);
        } else {
      ...
      return ethers.ethers.hexlify(new Uint8Array(signatureBuff));
    },
    clear: async () => {
...
const nameToId = Object.entries(network.networks).reduce((acc, [key, value]) => {
...
class SequenceWaaSBase {
  ...
  async initiateIdTokenAuth(idToken, exp) {
    const sessionId = await this.getSessionId();
    const idTokenHash = ethers.ethers.id(idToken);
    const intent = await initiateAuth({
      sessionId,
  ...
  async initiatePlayFabAuth(titleId, sessionTicket) {
    const sessionId = await this.getSessionId();
    const ticketHash = ethers.ethers.id(sessionTicket);
    const intent = await initiateAuth({
      sessionId,
...
class GuestChallenge extends Challenge {
  ...
  getIntentParams() {
    const answer = ethers.ethers.id(this.challenge + this.sessionId);
    return {
      identityType: IdentityType.Guest,
...
class EmailChallenge extends Challenge {
  ...
  setAnswer(answer) {
    this.hashedAnswer = ethers.ethers.id(this.challenge + answer);
  }
  withAnswer(answer) {
...
class IdTokenChallenge extends Challenge {
  ...
  getIntentParams() {
    const decoded = jwtDecode.jwtDecode(this.idToken);
    const idTokenHash = ethers.ethers.id(this.idToken);
    return {
      identityType: IdentityType.OIDC,
...
class PlayFabChallenge extends Challenge {
  ...
  getIntentParams() {
    const ticketHash = ethers.ethers.id(this.sessionTicket);
    return {
      identityType: IdentityType.PlayFab,
...
class SequenceWaaS {
  ...
  async initIdTokenAuth(idToken) {
    var _decoded$iss;
    const decoded = jwtDecode.jwtDecode(idToken);
  ...
  async getSessionHash() {
    ...
    return ethers.ethers.id(sessionId);
  }
  async dropSession({
    sessionId,
    strict
...
exports.AccountAlreadyLinkedError = AccountAlreadyLinkedError;
exports.AnswerIncorrectError = AnswerIncorrectError;
exports.Challenge = Challenge;
exports.ChallengeExpiredError = ChallengeExpiredError;
exports.EmailAlreadyInUseError = EmailAlreadyInUseError;
exports.EmailChallenge = EmailChallenge;
exports.FeeTokenType = FeeTokenType;
exports.GuestChallenge = GuestChallenge;
exports.IdTokenChallenge = IdTokenChallenge;
exports.IdentityType = IdentityType;
exports.IntentName = IntentName;
exports.IntentResponseCode = IntentResponseCode;
exports.PlayFabChallenge = PlayFabChallenge;
exports.ProofVerificationFailedError = ProofVerificationFailedError;
exports.SequenceWaaS = SequenceWaaS;
exports.SequenceWaaSBase = SequenceWaaSBase;
exports.StytchChallenge = StytchChallenge;
...
exports.defaultArgsOrFail = defaultArgsOrFail;
exports.delayedEncode = delayedEncode;
exports.erc1155 = erc1155;
exports.erc20 = erc20;
exports.erc721 = erc721;
exports.errors = errors;
exports.isCloseSessionResponse = isCloseSessionResponse;
exports.isFailedTransactionResponse = isFailedTransactionResponse;
exports.isFeeOptionsResponse = isFeeOptionsResponse;
exports.isFinishValidateSessionResponse = isFinishValidateSessionResponse;
exports.isGetIdTokenResponse = isGetIdTokenResponse;
exports.isGetSessionResponse = isGetSessionResponse;
exports.isInitiateAuthResponse = isInitiateAuthResponse;
exports.isIntentTimeError = isIntentTimeError;
exports.isLinkAccountResponse = isLinkAccountResponse;
exports.isListAccountsResponse = isListAccountsResponse;
exports.isMaySentTransactionResponse = isMaySentTransactionResponse;
exports.isOpenSessionResponse = isOpenSessionResponse;
exports.isSentTransactionResponse = isSentTransactionResponse;
exports.isSessionAuthProofResponse = isSessionAuthProofResponse;
exports.isSignedMessageResponse = isSignedMessageResponse;
exports.isTimedOutTransactionResponse = isTimedOutTransactionResponse;
exports.isValidateSessionResponse = isValidateSessionResponse;
exports.isValidationRequiredResponse = isValidationRequiredResponse;
exports.networks = networks;
exports.parseSequenceWaaSConfigKey = parseSequenceWaaSConfigKey;
exports.store = store;
