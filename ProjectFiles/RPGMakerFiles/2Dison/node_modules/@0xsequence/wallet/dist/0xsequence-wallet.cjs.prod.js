...
class Wallet extends ethers.ethers.AbstractSigner {
  ...
  static newWallet(options) {
    const address = core.commons.context.addressOf(options.context, options.coders.config.imageHashOf(options.config));
  ...
  reader() {
    ...
    return new core.commons.reader.OnChainReader(this.provider);
  }
  setConfig(config) {
    this.config = config;
  }
  ...
  async decorateTransactions(bundle) {
    ...
    const transactions = [{
      ...
      data: core.commons.transaction.encodeBundleExecData(decorated),
      revertOnError: true
    }];
  ...
  async buildDeployTransaction(metadata) {
    ...
    if (core.commons.context.addressOf(this.context, imageHash) !== this.address) {
  ...
  async signDigest(digest, metadata) {
    ...
    const subdigest = utils.subDigestOf(this.address, this.chainId, digest);
  ...
  async fetchNonceOrSpace(nonce) {
    ...
    return core.commons.transaction.encodeNonce(spaceValue, resultNonce);
  }
  ...
  async signTransactions(txs, nonce, metadata) {
    ...
    const digest = core.commons.transaction.digestOfTransactions(defaultedNonce, transactions);
    const meta = _extends({
      digest,
      transactions
    }, metadata);
    ...
    return {
      intent: {
        ...
        id: utils.subDigestOf(this.address, this.chainId, digest),
        wallet: this.address
      },
      chainId: this.chainId,
      transactions,
...
exports.SequenceOrchestratorWrapper = SequenceOrchestratorWrapper;
