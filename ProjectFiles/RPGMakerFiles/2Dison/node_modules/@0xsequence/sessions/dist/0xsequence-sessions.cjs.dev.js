...
class ArweaveReader {
  constructor(options = defaults) {
    ...
    if (options.eip5719Provider) {
      this.eip5719 = new replacer.CachedEIP5719(options.eip5719Provider);
    }
  }
  async loadPresignedConfiguration(args) {
    var _this = this;
    const wallet = ethers.ethers.getAddress(args.wallet);
    ...
    if (!core.v2.config.isWalletConfig(fromConfig)) {
    ...
    const items = Object.entries(await findItems({
      ...
      try {
        ...
        if (!ethers.ethers.isAddress(signer)) {
          throw new Error(`signer is not an address: ${signer}`);
        }
        if (!ethers.ethers.isHexString(subdigest, 32)) {
          throw new Error(`subdigest is not a hash: ${subdigest}`);
        }
        if (!ethers.ethers.isHexString(digest, 32)) {
        ...
        if (!ethers.ethers.isHexString(toImageHash, 32)) {
    ...
    for (let currentConfig = fromConfig; candidates.length;) {
      const currentImageHash = core.v2.config.imageHash(currentConfig);
      let nextCandidate;
      ...
      for (const candidate of candidates) {
        ...
        const {
          weight
        } = core.v2.signature.encodeSigners(currentConfig, new Map(nextCandidateSigners.map(signer => [signer, {
          signature: '0x',
          isDynamic: false
        }])), [], 0);
      ...
      try {
        ...
        if (!core.v2.config.isWalletConfig(nextConfig)) {
          throw new Error(`config ${nextImageHash} is not v2`);
        }
        const nextCandidateSignatures = new Map((await Promise.all(nextCandidateSigners.map(async function (signer) {
          const {
            id,
            subdigest,
            signatureType
          } = nextCandidateItems.get(signer);
          try {
            let signature = await (await fetchItem(id, _this.options.rateLimitRetryDelayMs, _this.options.arweaveUrl)).text();
            ...
            if (_this.eip5719) {
              try {
                signature = ethers.ethers.hexlify(await _this.eip5719.runByEIP5719(signer, subdigest, signature));
              } catch (error) {
                console.warn(`unable to run eip-5719 on config update ${id}`);
              }
            }
            const recovered = core.commons.signer.tryRecoverSigner(subdigest, signature);
            return [[signer, {
              signature,
        ...
        const {
          encoded: signature,
          weight
        } = core.v2.signature.encodeSigners(currentConfig, nextCandidateSignatures, [], 0);
        if (weight < BigInt(currentConfig.threshold)) {
  ...
  async configOfImageHash(args) {
    ...
    const config = async function (imageHash) {
      const items = Object.entries(await findItems({
        Type: 'config',
        Config: imageHash
      }, _this2.options)).flatMap(([id, tags]) => {
        try {
      ...
      switch (items.length) {
        case 0:
          _this2.configs.set(imageHash, Promise.resolve(undefined));
          return;
        case 1:
          break;
        default:
          console.warn(`multiple configs ${imageHash} at ${items.map(({
            id
          }) => id).join(', ')}`);
          break;
      }
      for (const {
        ...
        try {
          const config = _extends({}, await (await fetchItem(id, _this2.options.rateLimitRetryDelayMs, _this2.options.arweaveUrl)).json(), {
            version
          });
          if (config.tree) {
            config.tree = toTopology(config.tree);
          }
          const actual = core.universal.coderFor(version).config.imageHashOf(config);
          if (actual !== imageHash) {
            throw new Error(`image hash is ${actual}, expected ${imageHash}`);
          }
          _this2.configs.set(imageHash, Promise.resolve(config));
          return config;
        } catch (error) {
  ...
  async imageHashOfCounterfactualWallet(args) {
    ...
    const items = Object.entries(await findItems({
      ...
      try {
        ...
        const context = core.commons.context.defaultContexts[version];
        if (!context) {
          throw new Error(`unknown version: ${version}`);
        }
        if (core.commons.context.addressOf(context, imageHash) !== wallet) {
  ...
  async walletsOfSigner(args) {
    var _this3 = this;
    const signer = ethers.ethers.getAddress(args.signer);
    const proofs = new Map();
    for (const [id, tags] of Object.entries(await findItems({
      Type: ['signature', 'config update'],
      Signer: signer,
      Witness: args.allSignatures ? undefined : 'true'
    }, this.options))) {
      const {
        Wallet: wallet,
        Subdigest: subdigest,
      ...
      try {
        ...
        if (subdigest !== core.commons.signature.subdigestOf({
          digest,
          chainId,
          address: wallet
        })) {
        ...
        const signature = fetchItem(id, this.options.rateLimitRetryDelayMs, this.options.arweaveUrl).then(async function (response) {
          const signature = (await response.text()) + signatureType;
          if (_this3.eip5719) {
            try {
              return ethers.ethers.hexlify(await _this3.eip5719.runByEIP5719(signer, subdigest, signature));
            } catch (error) {
  ...
  async getMigration(address, fromImageHash, fromVersion, chainId) {
    const wallet = ethers.ethers.getAddress(address);
    const items = Object.entries(await findItems({
      Type: 'migration',
      Migration: wallet,
      ...
      try {
        ...
        if (!ethers.ethers.isAddress(executor)) {
        ...
        console.warn(`chain ${chainId} migration ${id} for v${fromVersion} wallet ${wallet} from config ${fromImageHash} invalid:`, error);
        return [];
      }
    });
    switch (items.length) {
    ...
    const [data, toConfig] = await Promise.all([fetchItem(id, this.options.rateLimitRetryDelayMs, this.options.arweaveUrl).then(response => response.text()), this.configOfImageHash({
      imageHash: toImageHash
    })]);
    if (!toConfig) {
      throw new Error(`unable to find to config ${toImageHash} for migration`);
    }
    const mainModule = new ethers.ethers.Interface(MAIN_MODULE_ABI);
    const [encoded, nonce, signature] = mainModule.decodeFunctionData('execute', data);
    const transactions = core.commons.transaction.fromTxAbiEncode(encoded);
    const subdigest = core.commons.transaction.subdigestOfTransactions(wallet, chainId, nonce, transactions);
    return {
      tx: {
        entrypoint: executor,
        transactions,
...
async function findItems(filter, options) {
  ...
  const tags = Object.entries(filter).flatMap(([name, values]) => values === undefined ? [] : [`{ name: "${namespace ? `${namespace}-${name}` : name}", values: [${typeof values === 'string' ? `"${values}"` : values.map(value => `"${value}"`).join(', ')}] }`]);
  const edges = [];
  for (let hasNextPage = true; hasNextPage && (maxResults === undefined || edges.length < maxResults);) {
    ...
    while (true) {
      ...
      console.warn(`rate limited by ${graphqlUrl}, trying again in ${rateLimitRetryDelayMs / 1000} seconds at ${new Date(Date.now() + rateLimitRetryDelayMs).toLocaleTimeString()}`);
  ...
  }) => [id, Object.fromEntries(tags.map(({
    name,
    value
  }) => [namespace && name.startsWith(`${namespace}-`) ? name.slice(namespace.length + 1) : name, value]))]));
}
async function fetchItem(id, rateLimitRetryDelayMs = defaults.rateLimitRetryDelayMs, arweaveUrl = defaults.arweaveUrl) {
  while (true) {
    const response = await fetch(`${arweaveUrl}/${id}`, {
      redirect: 'follow'
    });
    if (response.status !== 429) {
      return response;
    }
    console.warn(`rate limited by ${arweaveUrl}, trying again in ${rateLimitRetryDelayMs / 1000} seconds at ${new Date(Date.now() + rateLimitRetryDelayMs).toLocaleTimeString()}`);
    await new Promise(resolve => setTimeout(resolve, rateLimitRetryDelayMs));
  }
}
function toTopology(topology) {
  ...
  if (typeof topology === 'object' && (topology == null ? void 0 : topology.node) !== undefined) {
    return {
  ...
  if (core.v2.config.isNode(topology)) {
    return {
      left: toTopology(topology.left),
      right: toTopology(topology.right)
    };
  }
  if (core.v2.config.isNestedLeaf(topology)) {
    return _extends({}, topology, {
...
class Sessions {
  constructor(hostname, fetch) {
    ...
    this.saveSignerSignatures2 = (args, headers, signal) => {
