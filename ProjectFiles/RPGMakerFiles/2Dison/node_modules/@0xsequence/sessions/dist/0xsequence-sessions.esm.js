import { v2, commons, universal, v1 } from '@0xsequence/core';
  ...
  async loadPresignedConfiguration(args) {
    ...
    for (let currentConfig = fromConfig; candidates.length;) {
      ...
      for (const candidate of candidates) {
        ...
        const {
          weight
        } = v2.signature.encodeSigners(currentConfig, new Map(nextCandidateSigners.map(signer => [signer, {
          signature: '0x',
          isDynamic: false
        }])), [], 0);
      ...
      try {
        ...
        const nextCandidateSignatures = new Map((await Promise.all(nextCandidateSigners.map(async function (signer) {
          ...
          try {
            ...
            if (_this.eip5719) {
              try {
                signature = ethers.hexlify(await _this.eip5719.runByEIP5719(signer, subdigest, signature));
              } catch (error) {
  ...
  async walletsOfSigner(args) {
    ...
    for (const [id, tags] of Object.entries(await findItems({
      ...
      try {
        ...
        if (subdigest !== commons.signature.subdigestOf({
          digest,
          chainId,
          address: wallet
        })) {
        ...
        const signature = fetchItem(id, this.options.rateLimitRetryDelayMs, this.options.arweaveUrl).then(async function (response) {
          ...
          if (_this3.eip5719) {
            try {
              return ethers.hexlify(await _this3.eip5719.runByEIP5719(signer, subdigest, signature));
            } catch (error) {
  ...
  provider, store = new MemoryTrackerStore(), useEIP5719 = false) {
    ...
    this.loadPresignedConfiguration = async function (args) {
      ...
      const nextImageHashes = payloads.filter(p => (p == null ? void 0 : p.message) && (p == null ? void 0 : p.address) && p.address === wallet).map(p => ({
        payload: p,
        nextImageHash: v2.chained.decodeMessageSetImageHash(p.message)
      ...
      for (const entry of sortedNextConfigsAndCheckpoints) {
        ...
        const signatures = new Map((await Promise.all(signers.map(async function (signer) {
          ...
          const replacedSignature = ethers.hexlify(_this.useEIP5719 ? await _this.cachedEIP5719.runByEIP5719(signer, payload.subdigest, signature) : signature);
  ...
  async getMigration(address, fromImageHash, fromVersion, chainId) {
    ...
    const candidates = await Promise.all(txs.map(async function (tx) {
      ...
      const signatures = new Map((await Promise.all(signers.map(async function (signer) {
        ...
        const replacedSignature = ethers.hexlify(_this2.useEIP5719 ? await _this2.cachedEIP5719.runByEIP5719(signer, subdigest, signature) : signature);
              ...
              if (toConfig) {
                return {
                  ...
                  tx: {
                    ...
                    chainId,
                    intent: {
                      id: commons.transaction.subdigestOfTransactions(wallet, chainId, transactions.nonce, transactions.transactions),
                      wallet
                    }
                  }
                };
...
export { tracker, index as trackers };
