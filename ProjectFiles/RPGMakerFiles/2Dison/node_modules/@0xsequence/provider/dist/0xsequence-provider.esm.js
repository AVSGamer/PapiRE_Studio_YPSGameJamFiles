...
import { logger, encodeMessageDigest, encodeTypedDataDigest, sanitizeNumberString, sanitizeHost, sanitizeAlphanumeric, base64EncodeObject, bigintReplacer, base64DecodeObject, bigintReviver, toHexString } from '@0xsequence/utils';
import { findSupportedNetwork, ChainId, findNetworkConfig, allNetworks, JsonRpcProvider, loggingProviderMiddleware, exceptionProviderMiddleware, CachedProvider } from '@0xsequence/network';
import { signAuthorization } from '@0xsequence/auth';
import { isBrowser, Databeat } from '@databeat/tracker';

const PROHIBITED_FUNCTIONS = new Map(['addHook(bytes4,address)', 'clearExtraImageHashes(bytes32[])', 'removeHook(bytes4)', 'setExtraImageHash(bytes32,uint256)', 'updateIPFSRoot(bytes32)', 'updateImageHash(bytes32)', 'updateImageHashAndIPFS(bytes32,bytes32)', 'updateImplementation(address)'].map(signature => [ethers.id(signature).slice(0, 10), signature]));
    ...
    try {
      ...
      switch (request.method) {
        ...
        case 'eth_getTransactionCount':
          {
            const address = ethers.getAddress(request.params[0]);
...
function isJsonRpcProvider(provider) {
  return provider && typeof provider === 'object' && typeof provider.send === 'function' && typeof provider.getNetwork === 'function';
}
...
class SequenceProvider extends ethers.AbstractProvider {
  constructor(client, providerFor, networks = allNetworks, options) {
...
class Analytics extends Databeat {}
...
const initWallet = (projectAccessKey, partialConfig) => {
  ...
  const defaultNetwork = config.defaultNetwork ? (_findNetworkConfig = findNetworkConfig(combinedNetworks, config.defaultNetwork)) == null ? void 0 : _findNetworkConfig.chainId : undefined;
